...
func newNode() (*Node, error) {
	nodeHost, errID := newNodeID()
	if errID != nil {
		return nil, errID
	}

	dir := path.Join("data", nodeHost)
	...
	n := &Node{
		...
		ID:               nodeHost,
		Host:             nodeHost,
		ConsensusAddress: ipkit.NewAddr(nodeHost, config.ConsensusPort),
		GrpcAddress:      ipkit.NewAddr(nodeHost, config.GrpcPort),
		MainDir:          dir,
