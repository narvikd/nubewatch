...
//
// In nubewatch's particular case, since it uses BadgerDB, it already persists data when Apply is called,
//
...
func (dbFSM DatabaseFSM) Apply(log *raft.Log) any {
	...
	case raft.LogCommand:
		...
		case "ADD_SERVICE":
			data, err := dbFSM.addService(p.Key, p.Value)
			return &ApplyRes{
				Data:  data,
				Error: err,
			}
		case "SET_NOT_ALIVE_SERVER":
			return &ApplyRes{
				Error: dbFSM.setNotAliveServer(p.Key, p.Value),
			}
		case "UPDATE_LAST_CONTACT":
			return &ApplyRes{
				Error: dbFSM.updateLastContact(p.Key, p.Value),
			}
		default:
			return &ApplyRes{
...
//
// In the case of nubewatch, this method isn't needed because BadgerDB persists data when Apply is called.
//
...
//
// In the case of the nubewatch, Persist does nothing because BadgerDB already persists data when Apply is called.
...
//
// In the case of nubewatch, the Release method is empty because Persist does not acquire any resources.
