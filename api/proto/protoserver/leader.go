...
func (srv *server) ExecuteOnLeader(ctx context.Context, req *proto.ExecuteOnLeaderMsg) (*proto.ExecuteOnLeaderMsg, error) {
	log.Println("[proto] (ExecuteOnLeader) request received, processing...")

	// Applies the command to the leader
	result, errExecute := cluster.ApplyLeaderFuture(srv.Node.Consensus, req.Payload)
	if errExecute != nil {
		return &proto.ExecuteOnLeaderMsg{}, errExecute
	}

	j, errMarshal := json.Marshal(result)
	if errMarshal != nil {
		return nil, errorskit.Wrap(errMarshal, "couldn't marshal ExecuteOnLeader response")
	}

	log.Println("[proto] (ExecuteOnLeader) request successful")
	return &proto.ExecuteOnLeaderMsg{Payload: j}, nil
}
